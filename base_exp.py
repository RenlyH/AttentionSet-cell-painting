...
import pandas as pd
import numpy as np
from data_loader import generate_data_set, data_label_split, data_standardization
...
def train(size, data, model, verbose, parallel = True, bag_perc = 0.5):
    if parallel:
        results = multi_mini_noise_signal_cv(size, data, "taxol", "DMSO", model, 10, verbose, bag_perc)
    else:
        results = mini_noise_signal_cv(size, data, "taxol", "DMSO", model, 10, verbose, bag_perc)
    ...
    if os.path.exists("%s_sample%s_feature%s.csv" %(model_name, size, feature_size)):
        results.to_csv("%s_sample%s_feature%s.csv" %(model_name, size, feature_size), mode='a', header=False)
    else:
