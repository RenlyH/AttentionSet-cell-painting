...
def parallel(perc, data, splits, num_bag, bag_size_mean, bag_size_std, treatment, control, bag_perc, model, optimizer, epochs):
    ...
    for i, (train_index, test_index) in tqdm(enumerate(skf.split(X, y)), desc="%d fold cross validation"%splits):
        ...
        train_dataset = dmso_taxol_ProfileBag(X_train, num_bag, bag_size_mean, bag_size_std, perc, treatment, control, bag_perc)
        valida_dataset = dmso_taxol_ProfileBag(X_test, num_bag, bag_size_mean, bag_size_std, perc, treatment, control, bag_perc)
    ...
    return {perc:[np.mean(acc_control_list), np.std(acc_control_list), 
...
def mini_noise_signal_cv(data, num_bag, bag_size_mean, bag_size_std, treatment, control, model, optimizer, splits, epochs):
    ...
    results = [pool.apply_async(parallel, args=(i/100, data, splits, num_bag, bag_size_mean, bag_size_std, treatment, control, 0.5, model, optimizer, epochs)) for i in range(5,96,5)]
    results = [p.get() for p in results]
    dic = {}
    for i in results:
        dic.update(i)
    for i in sorted (dic): 
        mean_control_accuracy.append(dic[i][0])
        std_control_accuracy.append(dic[i][1])
        mean_treat_accuracy.append(dic[i][2])
        std_treat_accuracy.append(dic[i][3])
        mean_pred_score_control.append(dic[i][4])
        std_pred_score_control.append(dic[i][5])
        mean_pred_score_treatment.append(dic[i][6])
        std_pred_score_treatment.append(dic[i][7])
...
if __name__=='__main__':
    ...
    sf_deepset = SmallDeepSet()
    optimizer = optim.Adam(sf_deepset.parameters(), lr=0.0005, betas=(0.9, 0.999), weight_decay=10e-5)
    results = mini_noise_signal_cv(sf_drop_NA_data, 300, 100, 10, "taxol", "DMSO", sf_deepset, optimizer, 5, 2)
    ...
    results.columns = ["mean_control_accuracy", "std_control_accuracy", 
                ...
                "mean_pred_score_control", "std_pred_score_control",
                "mean_pred_score_treatment", "std_pred_score_treatment"]
    results.to_csv("exp_bagsize%d_full_feature.csv"%(300),index = False)
